  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        <Breadcrumb items={[{ label: 'MCQs', href: '/mcqs' }]} />
        <div className="mb-12 animate-fade-in">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
            NCERT Multiple Choice Questions <span className="text-blue-600">({CURRENT_ACADEMIC_YEAR})</span>
          </h1>
          <p className="text-lg text-gray-600 dark:text-gray-400 max-w-3xl">
            Chapter-wise multiple choice questions for all NCERT subjects based on the latest {CURRENT_ACADEMIC_YEAR} syllabus. Test your knowledge with interactive MCQs.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
          {classData.map((cls, index) => (
            <Link
              key={cls.slug}
              href={`/mcqs/${cls.slug}`}
              className="group relative p-6 rounded-2xl bg-gradient-to-br from-teal-500 to-teal-600 text-white overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-xl animate-fade-in"
