...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${cls.name} ${subject.name} Previous Year Questions (${CURRENT_ACADEMIC_YEAR}) - Chapter-wise`,
    description: `Get free chapter-wise CBSE previous year questions for ${cls.name} ${subject.name} with solutions. Based on NCERT ${CURRENT_ACADEMIC_YEAR} syllabus.`,
  };
}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <div className="mb-10 animate-fade-in">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
            {cls.name} {subject.name} <span className="text-blue-600">Previous Year Questions</span>
          </h1>
          <p className="text-lg text-gray-600 dark:text-gray-400 max-w-3xl">
            Chapter-wise CBSE previous year questions for {cls.name} {subject.name}. Practice questions from past board exams.
          </p>
        </div>
        ...
        {chapters.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 dark:text-gray-500 mt-2">PYQs are being added.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {chapters.map((chapter, index) => (
              <Link
                key={chapter.id}
                href={`/previous-year-questions/${cls.slug}/${subject.slug}/${chapter.slug}`}
                className="group p-5 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-rose-300 dark:hover:border-rose-600 transition-all hover:shadow-md animate-slide-up"
                style={{ animationDelay: `${index * 0.05}s` }}
              >
                <div className="flex items-start gap-3">
                  <div className="p-2 bg-rose-50 dark:bg-rose-900/30 rounded-lg flex-shrink-0">
                    <BookOpen className="h-5 w-5 text-rose-600 dark:text-rose-400" />
                  </div>
                  <div>
                    <h2 className="text-base font-semibold text-gray-900 dark:text-white group-hover:text-rose-600 transition-colors">
                      Chapter {chapter.chapterNumber}: {chapter.title}
                    </h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{chapter.description}</p>
                  </div>
                </div>
                <div className="flex items-center text-sm text-rose-600 dark:text-rose-400 font-medium mt-3 ml-10">
                  <span>View PYQs</span>
