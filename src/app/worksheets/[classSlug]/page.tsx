...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!cls) return {};
  return {
    title: `NCERT Worksheets for ${cls.name} (${CURRENT_ACADEMIC_YEAR}) - All Subjects`,
    description: `Get free printable worksheets for ${cls.name} based on NCERT ${CURRENT_ACADEMIC_YEAR} syllabus. Interactive practice worksheets for all subjects.`,
  };
}

export default async function WorksheetsClassPage({ params }: Props) {
  const { classSlug } = await params;
  const cls = getClassBySlug(classSlug);
  if (!cls) notFound();
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <div className="mb-10 animate-fade-in">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
            {cls.name} <span className="text-blue-600">Worksheets</span>
          </h1>
          <p className="text-lg text-gray-600 dark:text-gray-400 max-w-3xl">
            Interactive practice worksheets for {cls.name} covering all subjects based on the latest NCERT {CURRENT_ACADEMIC_YEAR} syllabus.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {cls.subjects.map((subject, index) => (
            <Link
              key={subject.id}
              href={`/worksheets/${cls.slug}/${subject.slug}`}
